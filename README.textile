...
p. *sort*
Sorts the values. If the attribute is used, all values are rearranged to the specified order. Available options are @regular@ (sorts without checking type), @numeric@ (sorts in numeric order), @string@ (sorts as strings) and @locale@ (sorts according server's locale settings), followed by sorting direction, either @desc@ and @asc@. By default the option isn't used (unset), and the values are returned in the order they were supplied.
