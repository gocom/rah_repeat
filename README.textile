...
The plugin comes with total of four tags. The main tag @rah_repeat@, a single tag @rah_repeat_value@, and two conditionals @rah_repeat_if_first@ and @rah_repeat_if_last@.
...
p. The tag, @<txp:rah_repeat>@, is the plugin's main tag. It's a container tag used to iterations, repeating and listing from the given values. Attributes for the tag follow.
...
p. Is a single tag, used to display the provided values. It should be used wrapped inside @<txp:rah_repeat></txp:rah_repeat>@ tags. Attributes for the tag follow.

p. *escape*
If set to @1@, HTML and Textpattern markup are escaped. HTML specialcharacters are converted to HTML entities. By default this option is used.
...
p. The tag, @<txp:rah_repeat_if_first>@, is a container tag, and has no attributes. It's a conditional tag that checks if the current items is the first one.
...
p. The tag, @<txp:rah_repeat_if_last>@, is a container tag, and has no attributes. It's a conditional tag that checks if the current items is the last one.
